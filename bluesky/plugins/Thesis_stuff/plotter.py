...
directory_all = os.listdir(r"bluesky\plugins\Thesis_stuff\output_all")
...
def conflictcounter(conflict_files):
    ...
    for scenario in conflict_files:
        conf_number_sub=[]
        for log in scenario:
            f= open(rf"bluesky\plugins\Thesis_stuff\output_all\{log}")
            data = pd.read_csv(rf"bluesky\plugins\Thesis_stuff\output_all\{log}", sep = ",", skiprows= 9,  header = None)
...
def fdcounter(cd_files, cdr_files):
    ...
    for i in range(len(cdr_files)):
        ...
        for j in range(len(cdr_files[i])):
            fd_number_sub_sub=[]
            f= open(rf"bluesky\plugins\Thesis_stuff\output_all\{cd_files[i][j]}")
            f2= open(rf"bluesky\plugins\Thesis_stuff\output_all\{cdr_files[i][j]}")
            data_cd = pd.read_csv(rf"bluesky\plugins\Thesis_stuff\output_all\{cd_files[i][j]}", sep = ",", skiprows= 9,  header = None)
            data_cd.columns = ["delt", "id", "spawnt", "flighttime", "dist", "2", "3", "lat","lon","6","7","8","9","10","11", "12", "13", "14"]
            data_cdr = pd.read_csv(rf"bluesky\plugins\Thesis_stuff\output_all\{cdr_files[i][j]}", sep = ",", skiprows= 9,  header = None)
...
plt.title("Cumulative distance flown")
plt.xticks([])
plt.yticks([])
